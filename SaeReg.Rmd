...
header-includes:
- \usepackage{babel}
- \AtBeginDocument{\renewcommand{\contentsname}{Tableau de Matières}}
---
...
knitr::opts_chunk$set(
  echo = TRUE,           # Hiển thị mã nguồn
  message = FALSE,       # Ẩn các thông báo
  warning = FALSE,       # Ẩn các cảnh báo
  highlight = FALSE,     # Không đánh màu syntax
  tidy = TRUE,           # Định dạng mã nguồn gọn gàng
...
```{r dataset,tidy=TRUE,tidy.opts=list(width.cutoff=60)}
...
```{r EDA,tidy=TRUE,tidy.opts=list(width.cutoff=60)}
...
```{r Clean,tidy=TRUE,tidy.opts=list(width.cutoff=60)}
...
```{r Total Voiture,tidy=TRUE,tidy.opts=list(width.cutoff=60)}
...
Tout d'abord, il est nécessaire de déterminer la variable indépendante (prédicteur) à inclure dans le modèle.
...
> **Conclusion:** À partir du RSS de ce méthode on peut dire que la premiere methode (retrouve dans 4.2.1) est plus optimise car le RSS de la premiere methode(1358) \> celle de la deuxieme methode (1390).
...
> **Conclusion:** Les deux méthodes du modèle logarithmique renvoient un RSS de 1281. Cela signifie que l'on peut efficacement utiliser l'une des deux méthodes.
...
### **4.3 Évaluation du modèle logarithmique.**
...
```{r 4 plot residual vs fitted,tidy=TRUE,tidy.opts=list(width.cutoff=60)}
...
Le modèle utilisé est maintenant : $$ y = a \cdot \log(x1+x2)+ b $$ avec $x1$ et $x2$ étant `displ` et `hwy`.
...
Le modèle utilisé est maintenant : $$ y = a \cdot  x1 + c \cdot x2 + b $$ avec $x1$ et $x2$ étant `displ` et `hwy`.
...
> **Conclusion:** Le modèle linéaire optimisé a réduit le RSS de \~ 5 fois par rapport au modèle linéaire précédent.
